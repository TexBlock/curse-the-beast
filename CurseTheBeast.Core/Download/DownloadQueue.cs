...
namespace CurseTheBeast.Core.Download;
...
public class DownloadQueue : IDisposable
{
    ...
    public async Task DownloadAsync(IEnumerable<FileEntry> tasks, CancellationToken ct = default)
    {
        ...
        CoreLog.Info(nameof(DownloadQueue), $"Queue started with {queue.Count} file(s).");
        ...
        await Task.WhenAll(Enumerable.Range(0, thread)
            ...
            .ToArray());
        CoreLog.Info(nameof(DownloadQueue), "Queue finished.");
    }
    ...
    async Task downloadWorker(ConcurrentQueue<FileEntry> queue, CancellationToken ct = default)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (file.Validate())
            {
                ...
                CoreLog.Verbose(nameof(DownloadQueue), $"Cache hit: {file.DisplayName ?? file.LocalPath}");
                continue;
            }
            TaskStarted?.Invoke(file);
            CoreLog.Info(nameof(DownloadQueue), $"Downloading: {file.DisplayName ?? file.LocalPath}");
            ...
            TaskFinished?.Invoke(file);
            CoreLog.Info(nameof(DownloadQueue), $"Finished: {file.DisplayName ?? file.LocalPath}");
        }
    }
    ...
    async ValueTask downloadFile(FileEntry file, Memory<byte> buffer, CancellationToken ct)
    {
        ...
        for (var i = 1; ; i++)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (i >= TryTimes)
                {
                    CoreLog.Error(nameof(DownloadQueue), $"Download failed: {uri}");
