...
public class DownloadCommand : AsyncCommand<DownloadCommand.Options>
{
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Options options)
    {
        ...
        HttpConfigService.SetupHttp(options.Thread, options.CurseforgeKey, options.NoProxy, options.Proxy, options.UserAgent);
        ...
        if(options.VersionId == 0)
        {
            ...
            GlobalStyle.Success.WriteLine($"√ Latest version id {options.VersionId} selected");
        ...
        GlobalStyle.Success.WriteLine($"√ {pack.Name} v{pack.Version.Name}({pack.Version.Type}) {packType}");
